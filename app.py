...
# Load the trained model
model = load_model('model/enhanced_SL_model3.h5')
...
def predict():
    ...
    # Convert to the correct format for OpenCV (BGR)
    frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)

    keypoints = preprocess_frame(frame)
    sequence_data = np.expand_dims(np.array([keypoints]), axis=0)  # Add batch dimension
    prediction = model.predict(sequence_data)
    predicted_class = np.argmax(prediction, axis=1)[0]

    return jsonify({'prediction': actions[predicted_class]})

# Optional: Add the video_feed route if needed
@app.route('/video_feed')
def video_feed():
    return Response(generate_frames(), mimetype='multipart/x-mixed-replace; boundary=frame')
